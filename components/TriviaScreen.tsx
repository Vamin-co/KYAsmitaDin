...
export function TriviaScreen({
  ...
  const attemptsLeft = solved ? 0 : 2 - attempts.length;

  async function submit(e: React.FormEvent) {
    e.preventDefault();
    if (busy || finished || !question || !input.trim()) return;
    setBusy(true);
    setError(null);
    ...
    <div className="space-y-4">
      ...
      {!question ? (
        <div className="bg-card rounded-2xl border border-line p-8 text-center animate-rise">
          <p className="text-ink font-semibold">No question is live right now</p>
